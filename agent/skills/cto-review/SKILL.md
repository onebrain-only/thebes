...
> **Note — Python helpers are not installed.** This skill references CLI calculators
> (`pmf_scorer.py`, `portfolio_analyzer.py`, `team_scaling_calculator.py`,
> `tech_debt_analyzer.py`, `rice_prioritizer.py`) that were deliberately not brought
> across: ~96KB of unread executable code is not worth the trust surface for
> arithmetic. **Apply the frameworks directly and show your working** — the scoring
> logic is described in the prose below. If a calculator turns out to earn its
> keep, we read that one script and install it alone.
